defmodule Rockelivery.Orders.Create do
  ...
  def call(params) do
    params |> fetch_items() |> handle_items(params)
  end
  ...
  defp fetch_items(%{"items" => items_params}) do
    ...
    query |> Repo.all() |> validate_and_multiply_items(items_ids, items_params)
  end
  ...
  defp multiply_items(true, _items, _items_params), do: {:error, "Invalid items"}
  ...
  defp handle_items({:ok, items}, params) do
    params |> Order.changeset(items) |> Repo.insert() |> handle_insert()
  end
